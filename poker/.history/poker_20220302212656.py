...
class Poker:
    ...
    def _extract_full_house(hand: list) -> Union[list[Card], None]:
        triple = Poker._extract_set(hand, 3)
        pair = Poker._extract_set(hand, 2)
    ...
    @staticmethod
    def find_set(hand: list, set_size: int) -> Card:
    ...
    @staticmethod
    def _remove_set(hand: list, set_size: int) -> Card:
        sets = [k for k, v in Counter(hand).items() if v == set_size]
        if len(sets) == 0:
            return None

        max_set_val = max(sets, default=None)
